...
Dygraph.Export.asCanvas = function (dygraph, userOptions) {
	...
	var labels = dygraph.attr_("labels").slice(1); // Drop the first
	// element, which is the label for the time dimension
	var legendHeight = ((labels.length / 2) + 1) * options.legendHeight; // half length, uncertainty plots
	// skipped.

    canvas.width = dygraph.width_;
    canvas.height = dygraph.height_ -options.rangeSelectorHeight + legendHeight;
...
Dygraph.Export.drawPlot = function (canvas, dygraph, options) {
    ...
    if (labelsPluginDict) {
        ...
        Dygraph.Export.putXLabel(ctx, dygraph, labelsPlugin.xlabel_div_, options, 
...
Dygraph.Export.putLabel = function (ctx, divLabel, options, font, color) {
    ...
    if (!divLabel.style.top.length) {
        ...
        top = ctx.canvas.height - legendHeight - bottom - height;
    }
...
/**
 * Draws x axis label just beneath the graph.
 */
Dygraph.Export.putXLabel = function (ctx, dygraph, divLabel, options, font, color) {
    "use strict";
    ...
    var top = top = dygraph.height_ - options.rangeSelectorHeight;
    var left = parseInt(divLabel.style.left, 10);
...
Dygraph.Export.drawLegend = function (canvas, dygraph, options) {
    ...
    var labelsX = 5;
    var labelsY = dygraph.height_ - options.rangeSelectorHeight + labelTopMargin;
    ...
    // uncertainty plots are skipped for the export to PNG.
    for (i = 0; i < labels.length; i += 2) {
        if (labelVisibility[i]) {
            ...
            usedColorCount += 2;// extra to skip gray color for uncertainty.
            ctx.fillText(txt, labelsX, labelsY);
            labelsY = labelsY + options.legendHeight;
        }
    }
};
